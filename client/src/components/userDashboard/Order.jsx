...
const Order = () => {
  ...
  if (loading) {
    return <div className="text-center text-lg text-secondary">Loading your orders...</div>;
  }

  if (!orders.length) {
    return <div className="text-center text-lg text-secondary">No orders found. Place a new order from the Order Now page.</div>;
  }
      ...
      {orders.map((order) => (
        <div key={order._id} className="rounded-md border border-base-200 bg-primary p-6 shadow-sm">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div>
              <p className="text-sm font-semibold text-primary-contemnt">Delivery Address</p>
          ...
          <div className="mt-5 rounded-3xl bg-primary-content p-4">
            ...
            <div className="space-y-2">
              {order.items.map((item) => (
                <div key={item.name} className="flex items-center justify-between rounded-2xl bg-base-100 px-4 py-3 text-sm shadow-sm">
                  <span>{item.name}</span>
