...
const OverView = () => {
  ...
  return (
    <div className='h-full overflow-auto bg-base-100 p-4 sm:p-6 lg:p-8'>
      <div className='rounded-3xl bg-gradient-to-r from-primary to-warning p-6 text-primary-content shadow-lg'>
        <div className='flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between'>
          ...
          <Link to='/order-now' className='inline-flex items-center justify-center gap-2 rounded-full bg-base-100 px-4 py-2 text-sm font-semibold text-primary transition hover:opacity-90'>
      ...
      <div className='mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4'>
        {stats.map((item, index) => (
          <div key={index} className='rounded-2xl border border-base-200 bg-base-100 p-4 shadow-sm flex justify-between'>
          ...
          <div className='mt-4 space-y-3'>
            {restaurants.slice(0, 3).map((restaurant) => (
              <div key={restaurant.id} className='flex items-center justify-between rounded-2xl border border-base-200 bg-base-100 p-4'>
                ...
                <Link to='/order-now' className='rounded-full bg-primary px-3 py-2 text-sm font-semibold text-primary-content'>View menu</Link>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className='space-y-6'>
          <div className='rounded-3xl border border-base-200 bg-base-100 p-6 shadow-sm'>
            ...
            <ul className='mt-4 space-y-3 text-sm text-secondary'>
              <li className='rounded-xl bg-base-100 p-3'>Your last order from <span className='font-semibold text-base-content'>Under The Mango Tree</span> was delivered 20 mins ago.</li>
              <li className='rounded-xl bg-base-100 p-3'>You saved <span className='font-semibold text-base-content'>3 restaurants</span> to your wishlist this week.</li>
              <li className='rounded-xl bg-base-100 p-3'>New offer available for <span className='font-semibold text-base-content'>free delivery</span> on your next order.</li>
            </ul>
          </div>

          <div className='rounded-3xl border border-base-200 bg-primary p-6 text-primary-content shadow-sm'>
