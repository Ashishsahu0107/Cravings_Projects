    ...
    <main className="bg-(--color-base-100)">
          ...
          {[
            ...
            <div key={label}>
              <p className="text-3xl font-bold text-(--color-primary)">
                {label === "Average Rating" && <FaStar className="mb-1 inline text-yellow-400" />} {value}
              </p>
      ...
      <section className="mx-auto grid max-w-5xl gap-12 px-6 py-16 md:grid-cols-2 md:items-center">
        ...
        <div className="grid grid-cols-2 gap-4">
          {[
            ...
            <div key={label} className="flex flex-col gap-2 rounded-xl border border-(--color-base-300) bg-white p-5 shadow-sm">
      ...
      <section className="bg-(--color-base-200) px-6 py-16">
        ...
        <div className="mx-auto grid max-w-5xl gap-8 md:grid-cols-3">
          {values.map((value) => (
            <div key={value.title} className="rounded-xl border border-(--color-base-300) bg-white p-8 text-center shadow-sm">
              <div className="mb-4 flex justify-center">{value.icon}</div>
              <h3 className="mb-2 text-lg font-bold text-(--color-neutral)">{value.title}</h3>
              <p className="text-sm leading-relaxed text-(--color-secondary)">{value.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-(--color-primary) px-6 py-14 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          <Link to="/register/customer" className="rounded-md bg-white px-8 py-3 font-semibold text-(--color-primary) hover:bg-orange-100">
            Get Started
          </Link>
          <Link to="/contact" className="rounded-md border-2 border-white px-8 py-3 font-semibold text-white hover:bg-white/10">
            Contact Us
          </Link>
        </div>
      </section>
